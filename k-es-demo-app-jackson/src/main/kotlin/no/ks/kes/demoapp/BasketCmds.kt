...
class BasketCmds(repo: AggregateRepository, paymentProcessor: PaymentProcessor) : CmdHandler<BasketAggregate>(repo, Basket) {
    ...
    init {
        init<Create> { Succeed(
            WriteEventWrapper( event = Basket.Created(it.aggregateId), aggregateId = it.aggregateId)) }

        apply<AddItem> {
            if (basketClosed)
                Fail(IllegalStateException("Can't add items to a closed basket"))
            else
                Succeed(WriteEventWrapper(event = Basket.ItemAdded(it.aggregateId, it.itemId), aggregateId = it.aggregateId))
        }
        ...
        apply<CheckOut> {
            when {
                ...
                basketContents.isEmpty() -> Fail(IllegalStateException("Can't check out a empty basket, buy something first?"))
                    ...
                    Succeed(
                        WriteEventWrapper( event = Basket.CheckedOut(it.aggregateId, basketContents.toMap()), aggregateId = it.aggregateId))
                } catch (e: Exception) {
