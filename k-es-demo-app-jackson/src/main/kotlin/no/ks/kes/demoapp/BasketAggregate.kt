...
object Basket : AggregateConfiguration<BasketAggregate>("basket") {
    ...
    data class SessionStarted(val aggregateId: UUID) : Event<BasketAggregate> {
    ...
    data class Created(val aggregateId: UUID) : Event<BasketAggregate>

    @SerializationId("BasketItemAdded")
    data class ItemAdded(val aggregateId: UUID, val itemId: UUID) : Event<BasketAggregate>

    @SerializationId("BasketCheckedOut")
    data class CheckedOut(val aggregateId: UUID, val items: Map<UUID, Int>) : Event<BasketAggregate>
}
